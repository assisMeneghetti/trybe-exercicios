...
router.get('/', async (_req, res) => {
  try {
    ...
    res.status(500).json({ message: algoDeuErrado });
  }
});
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: usuarioNaoEncontrado });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (!updateUser) return res.status(404).json({ message: usuarioNaoEncontrado });
